...
By default, everything with the filesystems of a container is ephemeral. During startup, each pod in a container gets a filesystem created based on the layers in its {{< glossary_tooltip text="container image" term_id="image" >}}. When a container is restarted or destroyed, that root filesystem, including any modifications to it, is lost.

A Kubernetes volume's lifecycle is tied to the *pod* that encloses it. The pod maintains the volume as different containers are started or stopped. Multiple volumes of any type may be connected to a pod. The [PersistentVolume](/docs/concepts/storage/persistent-volumes/) abstraction builds on the volume concept to let pods access storage that outlasts the lifetime of a pod.
...
Kubernetes supports a variety of different volume types, such as directories from the host system and NFS mounts. 

In addition to filesystem volumes, Kubernetes also supports volumes in a raw block access mode. For example, The `volumeMode` field can be set to `block` in the specifications for PersistentVolume and PersistentVolumeClaim. 
 
### Volume specification example
 
First, specify a pod that will provide the volume. This example uses the `emptyDir` volume type, and includes mounting the volume. 
...
### Core volume types {#volume-types-core}

Kubernetes supports several types of volumes. These core volume types are 
integrated into Kubernetes, and include network storage volume types and storage provided by the pod itself. 
...
This is powered by storage plugins. More specifically, Kubernetes implements the {{< glossary_tooltip text="Container Storage Interface" term_id="csi" >}} (CSI),
...
[FlexVolume](#flexvolume) is an another mechanism for storage plugins. Unlike CSI, FlexVolume is
...
Kubernetes directly implements some volume types for communicating with externally managed volumes, such as AWS EBS or Azure Disk Storage. These volume types are described as in-tree storage plugins. 
...
The `volumeMounts.subPath` property specifies a sub-path inside the referenced volume instead of its root. For example, this property may be used to to share one volume for multiple uses in a single pod.
...
and FlexVolume. They enable storage vendors to create custom storage plugins. The main Kubernetes binary implements the CSI, and communicates with any external plugins. This keeps the main binary efficent, and allows users to choose their own storage plugins. 
...
[PersistentVolume](/docs/concepts/storage/persistent-volumes/) into a pod. PersistentVolumeClaims are a way for cluster developers to "claim" durable storage (such as a GCE PersistentDisk or an iSCSI volume) without knowing the details of the particular cloud environment.
...
A `quobyte` volume allows an existing [Quobyte](https://www.quobyte.com) volume to be mounted into your pod.

Quobyte supports the {{< glossary_tooltip text="Container Storage Interface" term_id="csi" >}}. CSI is the recommended plugin to use Quobyte volumes inside Kubernetes. Quobyte's GitHub project has [instructions](https://github.com/quobyte/quobyte-csi#quobyte-csi) for deploying Quobyte using CSI, along with examples.
...
plugin allows deployed pods to access existing ScaleIO volumes (or it can dynamically provision new volumes for persistent volume claims, see
[ScaleIO Persistent Volumes](/docs/concepts/storage/persistent-volumes/#scaleio)).

You must have an existing ScaleIO cluster already setup and running with the volumes created before you can use them.
...
For further detail, see the [ScaleIO](https://github.com/kubernetes/examples/tree/{{< param "githubbranch" >}}/staging/volumes/scaleio) examples.
...
You must run the StorageOS container on each node that wants to access StorageOS volumes or that will contribute storage capacity to the pool.
...
For more information about StorageOS, dynamic provisioning, and PersistnetVolumeClaims, please see the
...
Learn more about managing storage declaratively using [Persistent Volumes](https://k8s.io/docs/concepts/storage/persistent-volumes/).
